...
Background::Background(std::uint16_t width, std::uint16_t height){
    this->height = height; 
    this->width = width;
    set_default_corners();
    // paint();
}
Background::Background(std::uint16_t width, std::uint16_t height, Pt bl, Pt br, Pt tl, Pt tr){
    this->height = height; 
    this->width = width;
    corn_bl = bl;
    corn_tl = tl;
...
Background::Pt::Pt(){ //defaults to origin
    x = 0;
    y = 0;
}
Background::Pt::Pt(std::uint16_t x,std::uint16_t y){
    this->x = x;
    this->y = y;
}
Background::Pt::Pt(std::uint16_t x,std::uint16_t y, Color color){
    this->x = x;
    this->y = y;
    this->color = color;
}

Background::Color::Color(){ //defaults to black
    r = 0;
    g = 0;
    b = 0;
}
Background::Color::Color(std::uint16_t r,std::uint16_t g, std::uint16_t b){
    this->r = r;
    this->g = g;
    this->b = b;
}
void Background::Pt::paint(Color color){
    this->color = color;
}
...
Background::Color Background::paint_one(Pt tgt){
...
void Background::paint(std::string file_name){
    ...
    out_file<<mn<<std::endl<<width<<' '<<height<<std::endl<<max_channel_value<<std::endl;
    for(std::uint16_t row{0}; row < height; ++row){
        for(std::uint16_t col{0}; col < width; ++col){
            if(row == corn_bl.y && col == corn_bl.x) out_file<<corn_bl.color;
            else if(row == corn_br.y && col == corn_br.x) out_file<<corn_br.color;
            else if(row == corn_tl.y && col == corn_tl.x) out_file<<corn_tl.color;
            else if(row == corn_tr.y && col == corn_tr.x) out_file<<corn_tr.color;
            else out_file<<paint_one({col,row});
        }
    }
    out_file.close();
};

Background::Color Background::sample(std::uint16_t width, std::uint16_t height){
    return paint_one({width, height});
}