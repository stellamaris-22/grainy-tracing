...
int main(int argc, char**argv){
    ...
    for(auto i{1}; i<argc; ++i){
        if(argv[i][0] != '-') ro.input_scene_file = argv[i];
        else {
            ro.print_help_msg();
            return 0;
        }
    ...
    // Background::Pt bl(0, height-1), br(width-1,height-1), tl(0,0), tr(width-1,0);
    ...
    // Background bkgd(width,height,bl,br,tl,tr);
